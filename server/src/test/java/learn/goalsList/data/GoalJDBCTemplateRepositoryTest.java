...
class GoalJDBCTemplateRepositoryTest {
    ...
    void findByUserId() {
        ...
        assertEquals("Vacation", allGoals.get(0).getName());
    }
    @Test
    ...
    void shouldCreateGoal() {
        ...
        assertEquals(3, actual.getGoalId());
    }

    @Test
    void updateGoal() {
        Goal goalToUpdate = new Goal();
        goalToUpdate.setGoalId(2);
        goalToUpdate.setName("Vacation2");
        goalToUpdate.setReason("updated goal");
        assertTrue(repository.updateGoal(goalToUpdate));
        assertEquals("updated goal", repository.findById(2).getReason());
    }

    @Test
    void deleteGoal() {
        assertTrue(repository.deleteGoal(2));
    }
